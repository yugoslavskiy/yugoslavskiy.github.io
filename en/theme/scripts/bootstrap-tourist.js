...
//			}
		...
		Tour.prototype._scrollIntoView = function (i) {
			...
			case 'left':
			case 'right':
				scrollTop = (offsetTop + height / 2) - (windowHeight / 2);
				break;
			case 'bottom':
				scrollTop = (offsetTop + height) - (windowHeight / 2);
			}
			// The site content scrolls inside .attack-website-wrapper (overflow-y: auto; max-height: 100vh),
			// not the window: animating body/html scrollTop does nothing there. Scroll whichever
			// container actually scrolls. Inside the wrapper, offset().top is a viewport position
			// (the window itself never scrolls), so the target is relative to the current scrollTop.
			var $scroller = $(".attack-website-wrapper");
			if ($scroller.length && $scroller[0].scrollHeight > $scroller[0].clientHeight + 1) {
				scrollTop = Math.max(0, $scroller.scrollTop() + scrollTop);
			} else {
				$scroller = $('body, html');
			...
			counter = 0;
			var expectedAnimations = $scroller.length;
			return $scroller.stop(true, true).animate({
				scrollTop: Math.ceil(scrollTop)
			}, (function (_this) {
					return function () {
						if (++counter === expectedAnimations) {
							_this._showPopoverAndOverlay(i);
