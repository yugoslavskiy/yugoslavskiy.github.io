// Выбор компоновки НЕ персистится: матрица всегда открывается во flat (дефолт
// разметки), переключатель действует в пределах открытой страницы. Раньше выбор
// сохранялся в localStorage (saved_layout) и восстановленный "side" из старых
// визитов перекрывал flat-дефолт — выглядело как сломанный дефолт.
...
function showSideMatrix() {
    ...
    layoutOptions.innerHTML = (typeof i18n_strings != 'undefined' && i18n_strings["js.matrix.layout.side"]) || "вид: сбоку";
}
...
function showFlatMatrix() {
    ...
    layoutOptions.innerHTML = (typeof i18n_strings != 'undefined' && i18n_strings["js.matrix.layout.flat"]) || "вид: плоский";
}
